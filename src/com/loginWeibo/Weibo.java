...
public class Weibo {
	...
	public static void main(String[] args) throws IOException {
		// System.out.println(getPass("liu920923",
		// "7734303538314989183@bjc|bgi|d|fj"));
		loginWeibo();
	}
	...
	static String getPass(String pw, String token) {
		String encodedPw = "";
		ScriptEngineManager manager = new ScriptEngineManager();
		ScriptEngine engine = manager.getEngineByName("javascript");
		try {
			engine.eval(new FileReader(new File("C:\\Users\\admin\\Desktop\\hali.js")));
		...
		if (engine instanceof Invocable) {
			Invocable in = (Invocable) engine;
			try {
				encodedPw = in.invokeFunction("get_pass", pw, token).toString();
			} catch (NoSuchMethodException e) {
				e.printStackTrace();
			} catch (ScriptException e) {
				e.printStackTrace();
	...
	static void loginWeibo() throws IOException {
		...
		String encodedUserName = new BASE64Encoder().encode(userName.getBytes("gbk"));
